        ...
        <div class="card">
            <div class="card-body">
                <div class="border p-4 rounded">
                    <form action="{{ route('marketer.report.update', $report->id) }}" method="POST">
                        @csrf
                        @method('put')
                        <div class="row">
                                ...
                                <select name="customer_id" class="form-control single-select">
                                    ...
                                    @foreach ($customers as $item)
                                        <option value="{{ $item->customer_id }}"
                                            {{ $report->customer_id == $item->customer_id ? 'selected' : '' }}>{{ optional($item->customer)->name }}
                                        </option>
                                    @endforeach
                                </select>
